...
give @s minecraft:diamond_sword{display: {Name: "My Custom Sword"'}} execute as f7a39418-72ca-4bf2-bc7e-ba9df67a4707 run say hello
...
execute as @e[nbt={ Item: {id: "minecraft:diamond", Count: 64 } }] execute if score @s foo< @s bar
scoreboard players operation @s foo %= @s bar
data get entity @s SelectedItem.tag.display.Name
data get entity @s Inventory[0]
data get entity @s Inventory[{id: "minecraft:diamond"}].Count data get entity @s Inventory[].tag{custom: true}.display.Name
...
tellraw @a [{"text": "hello", "color": "blue"}, {"text": "world", "color": "blue"}] execute as @a[scores={myscore=1..}]
